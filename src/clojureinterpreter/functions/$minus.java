 ...
 * @author 敲可爱
 */
public class $minus implements CloFunction{
    @Override
    ...
    //
    public Base invoke(Base[] params,int b) throws Exception{
        if(params==null||params.length-b==0)
            throw new IllegalArgumentException("no argument");
        if(params.length==1+b)
            return new Base(params[0+b].unwrap());
        //先拆包转化成string 再转化为double 可能有隐患
        Double ans = Double.parseDouble(params[b+0].unwrap().toString());
        for(int i=b+1;i<params.length;i++)
            ans -= Double.parseDouble(params[i].unwrap().toString());
        return new Base(ans);
    }
}
